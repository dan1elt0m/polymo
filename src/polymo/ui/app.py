"""FastAPI application powering the Polymo UI."""
...
def create_app() -> FastAPI:
    app = FastAPI(title="polymo", version=_polymo_version())
...
def _redact_secret(value: Any, needles: Sequence[str]) -> Any:
    ...
    token straight back into the preview UI. This only catches
...
def _resolve_preview_token(
    ...
    The UI passes the token typed into it separately from the
...
def _get_preview_df(
    *,
    ...
    Used only to infer/validate column dtypes for the UI; the
...
def _get_or_create_spark() -> Any:
    ...
    spark = (
        SparkSession.builder.appName("polymo-ui")
