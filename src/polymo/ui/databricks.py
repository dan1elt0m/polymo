"""Thin wrapper around the `databricks` CLI for read-only UI endpoints.

The UI shells out to the user's locally-installed `databricks` CLI
(which reads `~/.databrickscfg`) rather than talking to the Databricks REST
API directly. Every call here is read-only and stateless: the UI never
