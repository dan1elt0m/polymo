"""Run the generated fetch core in-process for UI previews."""
...
def _with_preview_state_path(
    ...
    machine running the UI, and a local one would make the preview depend on
...
def run_preview(
    ...
    Runs the same code the UI's "Generate script" action produces
...
def _run_preview(
    ...
    # emit a literal WINDOWS list; the UI preview blends records across
...
def _infer_field_types(records: List[Dict[str, Any]]) -> Dict[str, str]:
    ...
    runs against the preview records the UI already fetched.
    ...
    sampling ever comes up completely empty. For the UI preview it
