...
UI at an endpoint, describe pagination/auth/schema through a form,
...
- **No hand-written boilerplate.** Describe your API once through the
  point-and-click UI instead of writing pagination/retry/auth code by
  hand.
...
- **Designed for iterating.** Polymo autosaves your work-in-progress
  connectors locally so you can step away and pick up where you left off.

## Before you start
- Install Polymo: `pip install polymo`. This includes everything the UI
...
1. **Launch the UI.** Run `polymo --port 9000` and open the
   link it prints.
...
Polymo keeps a local library of every connector you work on. Use the
...
Full walkthrough: [UI](ui.md). Field-by-field reference for
...
- `polymo` launches a small web app (FastAPI + React) — the UI — that
...
- Build the dev-friendly image and launch the UI with hot reload:
