...
const filePickerSupported = !!(winRef && typeof winRef.showSaveFilePicker === 'function');
	...
	const loadConnector = React.useCallback(
		(connector: SavedConnector, options?: { message?: string }) => {
			...
			setActiveView(
				connector.activeView === 'code' || connector.activeView === 'deploy' ? connector.activeView : 'ui',
			);
	...
	const createConnector = React.useCallback(
		(options?: {
			...
			const connector: SavedConnector = {
				...
				lastEdited: 'ui',
				activeView: options?.activeView ?? 'ui',
	...
	const handleResumeWorkingState = React.useCallback(() => {
		...
		setActiveView(workingState.activeView);
	...
	}, [activeConnectorId, activeView, configFormState, lastEdited, readerOptions, setSavedConnectors, showLandingScreen]);
	...
	React.useEffect(() => {
		...
		return () => window.clearTimeout(handle);
	}, [activeConnectorId, activeView, configFormState, readerOptions, setWorkingState, showLandingScreen]);
	...
	const handleViewChange = React.useCallback(
		(value: string) => {
			setActiveView(value === "code" ? "code" : value === "deploy" ? "deploy" : "ui");
		},
		[setActiveView],
	);
	...
	const activeTabLabel = activeView === "code" ? "Generated code" : activeView === "deploy" ? "Deploy" : "Configuration";
	...
	const primaryPane = (
		...
		>
			<Tabs.Root value={activeView} onValueChange={handleViewChange} className="flex h-full min-h-0 flex-col">
				<div className="flex h-12 shrink-0 items-center justify-between gap-3 border-b border-border px-3">
					<Tabs.List className="inline-flex rounded-md border border-border bg-field p-0.5" aria-label="View">
						<Tabs.Trigger value="ui" className={tabTriggerClass} aria-label="Configuration">
							<span className="label-long">Configuration</span>
							<span className="label-short">Config</span>
						</Tabs.Trigger>
				...
				<div className="flex h-12 w-full items-center justify-between gap-4 px-4">
					<div className="flex min-w-0 items-center gap-2.5">
						...
						<div className="flex min-w-0 items-baseline gap-2 leading-tight select-none">
							<h1 className="text-sm font-semibold text-fg">Polymo</h1>
							{appVersion && (
