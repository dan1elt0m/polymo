...
def _page_response(page_index: int, extra_params: dict[str, str] | None) -> requests.Response:
    session = requests.Session()
    session.headers.update(HEADERS)
    params = dict(PARAMS)
    if extra_params:
...
def fetch_page(page_index: int, extra_params: dict[str, str] | None) -> list[dict[str, Any]]:
    """Fetch exactly one page of records."""
    response = _page_response(page_index, extra_params)
...
def _probe_total_pages(extra_params: dict[str, str] | None) -> int | None:
    """Fetch the first page and resolve the page count from the total hints."""
    response = _page_response(0, extra_params)
...
SCHEMA: str = "id BIGINT, state STRING, assignee_id BIGINT"

PUSHDOWN_PARAMS: dict[str, str] = {"state": "state", "assignee_id": "assignee"}


from typing import Iterable  # noqa: E402
from pyspark.sql.datasource import DataSource, DataSourceReader, EqualTo, Filter, InputPartition  # noqa: E402
...
class _Reader(DataSourceReader):
    ...
    def partitions(self) -> list[InputPartition]:
        total = _probe_total_pages(self._pushed)
    ...
    def read(self, partition) -> Iterator[tuple]:
        if partition.value is None:
            records = fetch_records(extra_params=self._pushed)
        else:
            records = fetch_page(partition.value, self._pushed)
